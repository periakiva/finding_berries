...
def mbox_iou(points1, points2):
    ...
    lt = torch.max(points1[:, None, :2], points2[:, :2])  # [N,M,2]
    rb = torch.min(points1[:, None, 2:], points2[:, 2:])  # [N,M,2]
...
class RegionProposalNetwork(torch.nn.Module):
    ...
    def assign_targets_to_anchors(self, anchors, targets):
        ...
        for anchors_per_image, targets_per_image in zip(anchors, targets):
            gt_points = targets_per_image["points"]
            
            match_quality_matrix = mbox_iou(gt_points, anchors_per_image)
    ...
    def forward(self, images, features, targets=None):
        ...
        # cvimg = np.transpose(images.tensors.cpu().detach().numpy().squeeze(),(1,2,0))
        # fig = plt.figure()
        ...
        # # cv2.waitKey(0)
        # plt.show()
        print(f"boxes after filter: {boxes[0].shape}\nscores after filter: {scores[0].shape}\nproposals after filter: {proposals[0].shape}\nscores: {scores[0][:50]}")
        # losses = {}
        # if self.training:
        #     assert targets is not None
        #     labels, matched_gt_boxes = self.assign_targets_to_anchors(anchors, targets)
        #     regression_targets = self.box_coder.encode(matched_gt_boxes, anchors)
        #     # print(f"objectsness: {objectness}, pred_bbox_deltas: {pred_bbox_deltas}, labels:{labels}, regression_targets:{regression_targets}")
        #     loss_objectness, loss_rpn_box_reg = self.compute_loss(
        #         objectness, pred_bbox_deltas, labels, regression_targets)
            
        #     loss_objectness = torch.clamp(loss_objectness,min=-100000,max=1000000)
        #     loss_rpn_box_reg = torch.clamp(loss_rpn_box_reg,min=-100000,max=100000)

            # if torch.isnan(loss_objectness):
            #     loss_objectness
            # print(f"labels: {torch.unique(labels[0])}, regresion targets: {regression_targets} loss_objectness: {loss_objectness}, loss_rpn_box_reg: {loss_rpn_box_reg}")
            # losses = {
