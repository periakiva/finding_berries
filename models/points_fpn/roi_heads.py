...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    # print(f"class logits: {class_logits.shape}, labels: {labels}")
    ...
    # print(f"box regression: {box_regression.shape}")
...
def maskrcnn_loss(mask_logits, proposals, gt_masks, gt_labels, mask_matched_idxs):
    ...
    # mask_prob = torch.sum(mask_prob[index, labels][:, None],dim=0)
    # if len(boxes_per_image) == 1:
    # # TODO : remove when dynamic split supported in ONNX
    #     # and remove assignment to mask_prob_list, just assign to mask_prob
    #     mask_prob_list = [mask_prob]
    # else:
    #     mask_prob_list = mask_prob.split(boxes_per_image, dim=0)
    # print(len(mask_prob_list))

    # plt.imshow(np.transmask_prob.detach().cpu().numpy())
    # plt.show()
    ...
    mask_loss = torch.clamp(mask_loss,min=-100000,max=1000000)
    # print(f"loss_objectness: {loss_objectness}, loss_rpn_box_reg: {loss_rpn_box_reg}")
    ### visalize tensor
    # if mask_loss < 0.5:
    #     print(f"mask loss: {mask_loss}, gt count: {len(gt_masks[0])}\npredicted count: {mask_logits.shape}")
    #     mask_prob = mask_logits.sigmoid()
    #     fig = plt.figure()
    #     ax1 = fig.add_subplot(2,2,1)
    #     ax1.imshow(mask_prob.cpu().detach().numpy()[0,1,:,:])
    #     ax1.title.set_text("proposal 0")
    #     ax2 = fig.add_subplot(2,2,2)
    #     ax2.imshow(mask_targets.cpu().detach().numpy()[0,:,:])
    #     ax2.title.set_text("GT 0")
    #     ax3 = fig.add_subplot(2,2,3)
    #     ax3.imshow(mask_prob.cpu().detach().numpy()[1,1,:,:])
    #     ax3.title.set_text("proposal 1")
    #     ax4 = fig.add_subplot(2,2,4)
    #     ax4.imshow(mask_targets.cpu().detach().numpy()[1,:,:])
    #     ax4.title.set_text("GT 1")
    #     plt.show()
    return mask_loss
...
class RoIHeads(torch.nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        for proposals_in_image, gt_boxes_in_image, gt_labels_in_image in zip(proposals, gt_boxes, gt_labels):
            ...
            # print(proposals_in_image.shape)
