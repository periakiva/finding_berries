...
class CBDatasetPoints(Dataset):
    ...
    def __getitem__(self,index):
        ...
        for i in range(num_objs):
            ...
            xmin = np.min(pos[1]) - 35
            xmax = np.max(pos[1]) + 35
            ymin = np.min(pos[0]) - 35
            ymax = np.max(pos[0]) + 35
            boxes.append([xmin, ymin, xmax, ymax])

         ### sanity check for boxes #####
        # cvimg = cv2.imread(img_path)
        # cvimg = np.asarray(image)q
        # fig = plt.figure()
...
class CBDatasetPointsPixelExpansion(Dataset):
    def __init__(self,directory,transformers=None,target_transformers=None,test=False,has_mask=True):
        ...
        self.csv_path = None
        self.csv_path = dictionary_contents(self.root_dir,types=["*.csv"])[0]
    ...
    def __getitem__(self,index):
        ...
        xy_list = self.csv_df.loc[self.csv_df['filename'] == img_name, 'locations'].iloc[0]
...
def get_model_instance_segmentation(num_classes):
    ...
    model = models.points_rcnn.mask_rcnn.maskrcnn_resnet50_fpn(pretrained=False,num_classes=2)
...
def cranberry_point_seg():
    ...
    # optimizer = torch.optim.SGD(params, lr=0.05,
    #                             momentum=0.9, weight_decay=0.0005)
    optimizer = torch.optim.Adam([dict(params=params, lr=0.0001),])
    # and a learning rate scheduler
    lr_scheduler = torch.optim.lr_scheduler.StepLR(optimizer,
                                                   step_size=3,
                                                   gamma=0.1)
